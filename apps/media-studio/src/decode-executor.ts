...
export function createDecodeExecutor(artifacts: RuntimeServices["artifacts"]): RuntimeExecutor {
  const decode = async function* (
    ...
    const blob = await Effect.runPromise(artifacts.getBlob(input));
    try {
  ...
  const decodeStreaming = async function* (
    ...
    try {
      ...
      try {
        ...
        if (!streamClosed) controller.error(new Error("decode cancelled"));
