...
export const DATA_MAX_BYTES = 16 * 1024 * 1024;
export const opfs = new OpfsStore("studio");
...
export async function readJsonArtifact<T>(ref: ArtifactRef, ctx: WorkerContext): Promise<T> {
...
export async function writeJsonArtifact(kind: string, payload: unknown): Promise<ArtifactRef> {
  return writeTypedJsonArtifact("document", kind, `document/${kind}`, payload);
}

export async function writeTypedJsonArtifact(
  namespace: string,
  kind: string,
  type: string,
  payload: unknown,
