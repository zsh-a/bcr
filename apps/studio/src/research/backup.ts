...
import { decodeResearch, type ResearchCollection, type ResearchLibrary } from "./index";
import { draftKey, readDraft } from "./management";
  ...
  return {
    ...
    collections: library.collections.map((collection) => ({
      ...
      excerpts: collection.excerpts.map((item) => ({
        ...
        ...(item.readerBindings
          ? {
              readerBindings: item.readerBindings.map(({ book, target, volume }) => ({
                book,
                target,
                ...(volume
                  ? { volume: { set: volume.set, index: volume.index, total: volume.total } }
                  : {}),
              })),
            }
          : {}),
