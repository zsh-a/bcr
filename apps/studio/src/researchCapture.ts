import { withTextCitation, type RuntimeMetadata } from "@bcr/core";
import type { ResearchCapture } from "@bcr/react";
import { ResearchStore, citationRoute, sameExcerpt, type ResearchExcerpt } from "./research";

const stores = new WeakMap<RuntimeMetadata, ResearchStore>();
/** Search and direct capture must share one commit queue and live snapshot. */
export function workspaceResearch(metadata: RuntimeMetadata | undefined): ResearchStore {
  if (!metadata) return new ResearchStore(undefined);
  let store = stores.get(metadata);
  if (!store) {
    store = new ResearchStore(metadata);
