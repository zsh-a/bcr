        ...
        <>
          ...
          <div className="space-y-1.5 px-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-sm text-text">{task.operation}</span>
            ...
            {task.error !== undefined && (
              <p className="rounded-sm border border-danger/30 bg-danger/10 px-2 py-1 font-mono text-xs text-danger">
                {task.error}
              </p>
            )}
          </div>
            ...
            <>
              ...
              <div className="space-y-2 px-3">
                {task.outputs.map((ref) => (
                  <div
                    key={ref.id}
                    className="rounded-sm border border-border bg-raised px-2 py-1.5"
                  >
                    <div className="truncate font-mono text-xs text-accent">{ref.id}</div>
                    <div className="mt-0.5 flex flex-wrap gap-x-3 gap-y-0.5 font-mono text-xs text-faint">
                      <span>{ref.type}</span>
...
function Row(props: { k: string; v: string; mono?: boolean }) {
  return (
    <div className="flex items-baseline gap-2 text-xs">
      <span className="w-14 shrink-0 font-mono text-xs text-faint">{props.k}</span>
      <span
