...
import { applyGlossaryTerms } from "./glossary";
import {
...
import { configureMangaTransformersCache } from "./model-cache";

export function createMangaCompute(io: ArtifactIO) {
  const { readJsonArtifact, writeTypedJsonArtifact } = io;
  ...
  async function mangaOcrOnnx(
    ...
    const blob = await io.getBlob(input);
  ...
  async function loadMangaTranslator(
    ...
    const loading = (async (): Promise<LoadedModel<MangaTranslator>> => {
      try {
        ...
        if (requested.device !== "webgpu") throw error;
        console.warn(
          "[manga-translate] WebGPU initialization failed, falling back to WASM:",
          error,
        );
      ...
      for (let offset = 0; offset < pending.length; offset += BATCH_SIZE) {
        ...
        ctx.progress(
          Math.min(
            0.98,
            0.4 +
              (0.58 * Math.min(pending.length, offset + batch.length)) /
