...
`AgentProvider` 为宿主持有独立的能力和编辑目标注册表，不使用模块级注册单例。编辑目标替换后，旧实例的清理不能注销新实例。框架无关的 `createAgentSession` 负责上下文、工具授权、审批和版本校验；React 适配器只转换消息并订阅事件，不再轮询流式文本。工具定义具有类型约束，执行上下文携带调用 ID 和取消信号；长任务需要主动响应信号。执行结果保留为聊天工具消息，在下一轮转换为带标记的数据文本；当前 WASM 消息接口仍是文本协议，不声称支持完整的原生工具历史重放。

领域导航通过 `NavigationService` 读取一致的位置快照；嵌入时由 Router 的完成事件驱动，独立运行时由浏览器 History 驱动。调用方不再手工广播导航事件。同一引用再次打开也有独立的导航修订号。

公共主题由 `@bcr/react/theme.css` 提供，`base.css` 仅由独立启动入口加载。领域视图不修改 `body` 的布局与背景；Media、DocGen 的通用控件选择器限制在领域根节点，领域容器建立独立层叠上下文。不同领域可以保留自己的强调色和信息布局。
