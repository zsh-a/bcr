...
- 连接元数据与凭据独立管理：地址和模型自动保存；密钥默认仅本页，可主动选择标签页或设备记忆。接口地址变化不携带旧密钥，并清理旧目标副本；不允许 URL 内嵌账号、密码、查询参数或片段。设备记忆明文存于浏览器，同源脚本可读取，不能防御 XSS；已打开的其他标签页不会被远程登出。存储失败明确提示，业务存档/导出不包含凭据。
...
对话采用紧凑的工作区/会话栏，模型接口入口位于输入区。连续且明确标记为 `read_only` 的工具调用投影为可展开轨迹；正文、错误、审批、写操作和旧版未知风险记录都是分组边界，不改变执行顺序。风险快照只用于展示，不能授权执行。

审批默认展示去除首尾相同行后的变更片段，并提供完整内容入口；长预览明确提示截取。领域可通过 `presentation.approvalLabel` 提供准确的动作名称。知识库保存提示只根据成功工具回执显示。低高度窗口采用整体滚动，避免输入区遮挡审批控件。

### 跨领域编辑知识库

知识库共享 `knowledge_create_note` 和 `knowledge_update_note`，不要求打开知识库或启用当前编辑器上下文。可以直接说“把以上内容保存为笔记”或“找到某篇笔记并补充这段内容”。工具先通过只读 `preview` 生成标题、集合、标签和正文的前后对照；用户批准后才写入。预览随审批记录保存，恢复对话不会重新执行写入。

更新工具使用检索/读取结果中的完整 `revision`，不是仅表示正文的 `version`；未提供的字段、来源引用保持不变。执行时在保存队列内再次检查版本、未保存草稿和同步冲突，过期修改拒绝覆盖。新建使用稳定 `requestId`，相同请求和内容可安全重试；同一 ID 内容不一致时拒绝创建。保存回执表示已持久化到本机，不表示已同步到 GitHub。

领域工具通过通用审批预览接口扩展，无需在聊天组件内加入知识库执行逻辑。
...
`verify-knowledge-agent-writes.mjs` 覆盖未打开知识库时的创建/更新、拒绝审批、重复请求、移动端审批和刷新后的持久化内容。

`verify-credentials.mjs` 覆盖连接恢复、凭据保存范围、独立标签页、目标绑定、清除和存储失败；浏览器清理数据、隐私模式、切换站点地址或端口会影响记忆能力。不提供跨设备同步、主密码加密或服务端凭据托管。

暂不引入分支对话、多任务队列、多 Agent 编排、消息虚拟化或自动上下文压缩。这些应在实际需求和长会话性能数据出现后，沿宿主记录与展示扩展点增量实现，不提前建立另一套状态体系。

连接设置是浮窗内的独立视图：对话保持挂载但不可交互，返回时恢复焦点；设置内容独立滚动，保存区固定。已有连接先展示摘要，显式进入编辑，取消不写入配置。返回时若有未保存修改需确认；清除密钥与删除连接放在管理区并二次确认。

AI 与 GitHub 复用 `ConnectionSummary`、`SecretField` 和 `CredentialScopeField`。已有密钥只显示配置状态，不回填输入框；替换时才允许输入及显示/隐藏。目标身份规范化后再判断是否改变，地址尾斜线或仓库大小写调整不会误清除凭据。所有存储仍由宿主服务负责，展示组件不直接访问浏览器存储。
