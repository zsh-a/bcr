...
export function createSurfaceRegistry() {
   ...
   */
  function registerSurface(surface: AgentSurface): () => void {
    surfaces.set(surface.kind, surface);
    emit();
    return () => {
      if (surfaces.get(surface.kind) !== surface) return;
      surfaces.delete(surface.kind);
      if (active === surface.kind) active = null;
      emit();
    };
  }

  function activateSurface(kind: string | null): void {
  ...
  function activeSurface(): AgentSurface | null {
   ...
   */
  function surfaceSummary(): SurfaceSummary | null {
  ...
  function surfaceRevision(): number {
    return revision;
  }
